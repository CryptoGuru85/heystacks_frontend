  ...
  >
    <div class="w-full h-full relative pt-12 px-4 sm:px-6 relative">
      ...
      <h3
        ...
        :style="state.layoutData.onMobile ? {'font-size': '1.5rem!important', 'line-height': '2.0rem'} : {'font-size': '1.725rem!important'}"
      >
      ...
      <div class="text-center" v-if="state.layoutData.page === 'main'">
        ...
        <button
          ...
          :style="state.layoutData.onMobile ? { 'font-size': '0.8rem!important', 'min-width': '235px' } : { 'font-size': '1.0rem!important', 'min-width': '285px' }"
      ...
      <div v-if="state.layoutData.page === 'existing'" class="sm:px-10 text-center">
        <a @click="state.layoutData.page = 'main'" class="mb-4 hover:underline text-emerald-800 active:text-emerald-600 cursor-pointer">
        ...
        <div class="form-group mb-3">
          <input
            ...
            :error="state.newDocumentObj.errors.indexOf('url-error') >= 0 || state.newDocumentObj.errors.indexOf('no-url') >= 0"
            filled
            required
          />
          <label 
            for="url-input"
            :class="(state.newDocumentObj.errors.indexOf('url-error') >= 0 || state.newDocumentObj.errors.indexOf('no-url') >= 0) ? `text-orange-600` : `text-default`"
          >Url</label>
          <p v-if="state.newDocumentObj.errors.indexOf('url-error') >= 0" class="text-red-500 text-left">
            Incorrect url - please double check your link. It should be something like "https://docs.google.com/document/d/1wf9YFtLFM4LuNkDzbb4hGfZqvb6VnKzJ9iZ"
          </p>
        </div>
        ...
        <div class="min-h-26">
          <transition name="fade">
            <small v-if="state.layoutData.comboboxInput && state.newDocumentObj.tagCandidate && state.newDocumentObj.tagCandidate !== 'No data available'" class="text-left mb-0 z-20">
              Enter:
        ...
        <button
          class="mt-5 mb-4 py-2 rounded w-full duration-200 bg-orange-600 hover:bg-orange-500 active:bg-orange-400"
          :disabled="state.layoutData.addLoading"
          @click="addDocument()"
        >
          <i v-if="!state.layoutData.addLoading" class="fa fa-plus text-xl mr-2 text-white"></i>
      ...
      <div v-if="state.layoutData.page === 'new'" class="text-center pb-6">
        ...
        <div class="flex justify-center">
          ...
          >
            <svg alt="Google" class="rounded mr-1 w-6" xmlns="http://www.w3.org/2000/svg" version="1.0" viewBox="0 0 64.000000 64.000000" preserveAspectRatio="xMidYMid meet">
        ...
        <img alt="Share doc 1" src="/src/assets/img/nd1.jpg" class="w-3/4 block mx-auto">
        <br>
        <img alt="Share doc 2" src="/src/assets/img/nd2.jpg" class="w-3/4 block mx-auto">

        <p class="subheading font-normal text-lg mb-2 mt-7 text-default">
          3. Add the link to heystacks: <a @click="state.layoutData.page = 'existing'" class=" hover:underline text-emerald-800 active:text-emerald-600 cursor-pointer">here</a>
        </p>
  ...
  setup(props, { emit }) {
    ...
    const addDocument = () => {
      ...
      if (!state.newDocumentObj.errors.length) {
        ...
        if (route.params.spaceName) docObjCopy.space = route.params.spaceName
        else if (props.space) docObjCopy.space = props.space
        fetch('/api/documents/add', {
          method: 'POST',
          body: JSON.stringify(docObjCopy)
        }).then(r => {
          if (r.status !== 200) {
            r.json().then(r => {
            ...
            r.json().then(r => {
              if (!r.success) {
                ...
                if (r.reason === 'url-error') alert('⚠️ Url error - please check if your url is in the correct format')
                if (r.reason === 'cant-access') alert('⚠️ Unable to access this document - are you sure this is a public document?')
              } else {
    ...
    const addTag = () => {
      if (state.newDocumentObj.tags.length > 7) {
        ...
        if (state.layoutData.comboboxInput && state.newDocumentObj.tags.map(tag => tag.text).indexOf(state.layoutData.comboboxInput) < 0) { state.newDocumentObj.tags.push({ text: state.layoutData.comboboxInput, value: state.layoutData.comboboxInput }) }
    ...
    const updateTagCandidate = () => {
      state.newDocumentObj.tagCandidate = (document.querySelector('.v-autocomplete__content > .v-list > .v-list-item--highlighted') || { textContent: 'No data available' }).textContent
    }

    const insertEmoji = (emoji) => {
      const el = new_modal_description.$el.querySelector('textarea')
      let cursorPos = el.selectionEnd
      state.newDocumentObj.description = state.newDocumentObj.description.substring(0, cursorPos) + emoji + state.newDocumentObj.description.substring(cursorPos)
      cursorPos += 1
      nextTick(() => {
